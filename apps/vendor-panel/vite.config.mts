...
export default defineConfig(({ mode }) => {
  ...
  const ALLOWED_HOSTS = env.VITE_ALLOWED_HOSTS || "http://localhost:5173"
  const ALLOWED_API = env.VITE_ALLOWED_API || "http://localhost:9000"
  ...
  return {
    ...
    server: {
      // open: false,
      allowedHosts: [ALLOWED_HOSTS, ALLOWED_API],
    },
    optimizeDeps: {
      entries: [],
      include: ["recharts"],
